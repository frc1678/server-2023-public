...
class ScoutPrecisionCalc(BaseCalculations):
    ...
    def calc_sim_precision(self, sim, tba_match_data):
        ...
        for calculation, schema in self.sim_schema["calculations"].items():
            ...
            aim_scout_errors = self.get_aim_scout_avg_errors(
                aim_scout_scores, tba_aim_score, sim["match_number"], sim["alliance_color_is_red"]
            )
            if aim_scout_errors == {}:
                continue
    ...
    def run(self):
        ...
        for entry in entries:
            sims.append(
                {"scout_name": entry["o"]["scout_name"], "match_number": entry["o"]["match_number"]}
            )
        ...
        for update in self.update_sim_precision_calcs(sims):
            self.server.db.update_document(
                "sim_precision",
                update,
                {"scout_name": update["scout_name"], "match_number": update["match_number"]},
            )
