...
class PredictedTeamCalc(BaseCalculations):
        ...
        for team_data in ranking_data:
            ...
            if team_number == int(team_data['team_key'][3:]):
                ...
                if team_data['matches_played'] > 0:
                    current_values['current_avg_rps'] = team_data['extra_stats'][0] / team_data['matches_played']
                else:
    ...
    def calculate_predicted_alliance_rps(self, predicted_aims):
        ...
        for match in match_list:
            ...
            for aim in range(2):
                ...
                if aims_in_match[aim]['predicted_score'] > aims_in_match[(aim + 1) % 2]['predicted_score']:
                    rps += 2
                elif aims_in_match[aim]['predicted_score'] == aims_in_match[(aim + 1) % 2]['predicted_score']:
                    rps += 1
                else:
                    rps += 0
    ...
    def update_predicted_team(self, predicted_aim):
        updates = []
        ranking_data = tba_communicator.tba_request(f'event/{self.server.TBA_EVENT_KEY}/rankings')['rankings']
        ...
        for team in teams:
            ...
            predicted_rps = self.calculate_predicted_team_rps(team, aim_list, predicted_alliance_rps)
    ...
    def run(self):
        ...
        self.server.db.insert_documents('predicted_team', self.update_predicted_team(predicted_aim))
