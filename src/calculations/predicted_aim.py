...
class PredictedAimCalc(BaseCalculations):
    ...
    def calculate_stage_contribution(self, predicted_values):
        ...
        auto_balls = sum([value for field, value in predicted_values.items() if 'auto_balls' in field])
        ...
        total_balls += sum([value for field, value in predicted_values.items() if 'tele_balls' in field])
    ...
    def calculate_predicted_balls_score(self, predicted_values, obj_team, tba_team):
        ...
        predicted_values.auto_balls_inner += obj_team['auto_avg_balls_high'] * tba_team['auto_high_balls_percent_inner']
        predicted_values.auto_balls_outer += obj_team['auto_avg_balls_high'] * (1 - tba_team['auto_high_balls_percent_inner'])

        # Find the predicted balls scored in tele
        predicted_values.tele_balls_low += obj_team['tele_avg_balls_low']
        predicted_values.tele_balls_inner += obj_team['tele_avg_balls_high'] * tba_team['tele_high_balls_percent_inner']
        predicted_values.tele_balls_outer += obj_team['tele_avg_balls_high'] * (1 - tba_team['tele_high_balls_percent_inner'])
    ...
    def calculate_predicted_alliance_score(self, predicted_values, obj_team_data, tba_team_data, team_numbers):
        ...
        for team in team_numbers:
            obj_team = [team_data for team_data in obj_team_data if team_data['team_number'] == team][0]
            tba_team = [team_data for team_data in tba_team_data if team_data['team_number'] == team][0]
            ...
            predicted_values.auto_line_success_rate += tba_team['auto_line_successes'] / obj_team['matches_played']
            predicted_values.climb_success_rate += tba_team['climb_all_successes'] / obj_team['matches_played']
            predicted_values.park_success_rate += tba_team['park_successes'] / obj_team['matches_played']
        ...
        for data_field in dataclasses.asdict(predicted_values).keys():
            if data_field == 'rotation_success_rate':
                if threshold_contribution >= self.ROTATION_THRESHOLD:
                    total_score += predicted_values.rotation_success_rate * self.POINTS['rotation_success_rate']
            elif data_field == 'position_success_rate':
                if threshold_contribution >= self.POSITION_THRESHOLD:
                    total_score += predicted_values.position_success_rate * self.POINTS['position_success_rate']
                ...
                total_score += math.ceil(predicted_values.park_success_rate - 0.5) * self.POINTS['park_success_rate']
            else:
    ...
    def calculate_predicted_climb_rp(self, predicted_values):
        ...
        """
        endgame_score = 0
        endgame_score += math.floor(predicted_values.climb_success_rate + 0.5) * self.POINTS['climb_success_rate']
        # If there are more than 2 climbs, assume balanced climb
        if endgame_score >= self.POINTS['climb_success_rate'] * 2:
            endgame_score += self.POINTS['level_bonus']
        endgame_score += math.ceil(predicted_values.park_success_rate - 0.5) * self.POINTS['park_success_rate']
    ...
    def update_predicted_aim(self, aims_list):
        ...
        for aim in aims_list:
            ...
            update = {'match_number': aim['match_number'], 'alliance_color_is_red': aim['alliance_color'] == 'R'}
            update['predicted_score'] = self.calculate_predicted_alliance_score(predicted_values, obj_team, tba_team, aim['team_list'])
    ...
    def run(self):
        ...
        for update in self.update_predicted_aim(aims):
            self.server.db.update_document(
                'predicted_aim', update, {'match_number': update['match_number'], 'alliance_color_is_red': update['alliance_color_is_red']}
            )
