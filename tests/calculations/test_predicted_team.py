...
class TestPredictedTeamCalc:
    ...
    def test_predicted_team_rps(self):
        assert self.test_calc.calculate_predicted_team_rps(
            1678, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(
            1533, self.aim_list, self.predicted_alliance_rps) == 7
        assert self.test_calc.calculate_predicted_team_rps(
            7229, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(
            254, self.aim_list, self.predicted_alliance_rps) == 2
        assert self.test_calc.calculate_predicted_team_rps(
            971, self.aim_list, self.predicted_alliance_rps) == 1
        assert self.test_calc.calculate_predicted_team_rps(
            1323, self.aim_list, self.predicted_alliance_rps) == 2
        assert self.test_calc.calculate_predicted_team_rps(
            2056, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(
            1114, self.aim_list, self.predicted_alliance_rps) == 6
        assert self.test_calc.calculate_predicted_team_rps(
            7179, self.aim_list, self.predicted_alliance_rps) == 6
