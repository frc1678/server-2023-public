...
class TestPredictedTeamCalc:
    def setup_method(self, method):
        ...
        self.aim_list = [{'match_number': 1, 'alliance_color': 'R', 'team_list': [1678, 1533, 7229]},
        ...
        self.updates = [{'team_number': 1678, 'predicted_rps': 5},
    ...
    def test_calculate_current_values(self):
        current_values = self.test_calc.calculate_current_values(self.ranking_data['rankings'], 1678)
    ...
    def test_predicted_team_rps(self):
        assert self.test_calc.calculate_predicted_team_rps(1678, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(1533, self.aim_list, self.predicted_alliance_rps) == 7
        assert self.test_calc.calculate_predicted_team_rps(7229, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(254, self.aim_list, self.predicted_alliance_rps) == 2
        assert self.test_calc.calculate_predicted_team_rps(971, self.aim_list, self.predicted_alliance_rps) == 1
        assert self.test_calc.calculate_predicted_team_rps(1323, self.aim_list, self.predicted_alliance_rps) == 2
        assert self.test_calc.calculate_predicted_team_rps(2056, self.aim_list, self.predicted_alliance_rps) == 5
        assert self.test_calc.calculate_predicted_team_rps(1114, self.aim_list, self.predicted_alliance_rps) == 6
        assert self.test_calc.calculate_predicted_team_rps(7179, self.aim_list, self.predicted_alliance_rps) == 6
    ...
    def test_update_predicted_team(self):
        with mock.patch('data_transfer.tba_communicator.tba_request', return_value=self.ranking_data),\
             mock.patch('calculations.predicted_team.PredictedTeamCalc._get_aim_list', return_value=self.aim_list),\
             mock.patch('calculations.predicted_team.PredictedTeamCalc._get_teams_list', return_value=self.teams):
