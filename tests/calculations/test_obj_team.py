...
def test_averaging_low_and_high_goals_together():
    ...
    tims = [{'auto_balls_low': 1, 'auto_balls_high': 2, 'tele_balls_low': 3, 'tele_balls_high': 4},
            {'auto_balls_low': 5, 'auto_balls_high': 6, 'tele_balls_low': 7, 'tele_balls_high': 8,
             'the coolest': 'Eugene Chen'}]
    schema_for_averages = {'auto_avg_balls_total': {'tim_fields': ['auto_balls_low', 'auto_balls_high'], 'type': 'float'},
                           'tele_avg_balls_total': {'tim_fields': ['tele_balls_low', 'tele_balls_high'], 'type': 'float'}}
    expected_output = {'auto_avg_balls_total': 7., 'tele_avg_balls_total': 11.}
...
def test_calculating_avg_incap_time():
    ...
    tims = [{'incap': 0.}, {'incap': 0.}, {'incap': 118}, {'incap': 0}]
...
def test_counts():
    ...
    tims = [{'control_panel_rotation': True,  'control_panel_position': True,  'climb_time': 0},
    ...
    schema_for_counts = {'tele_cp_rotation_successes': {'tim_fields': {'control_panel_rotation': True}, 'type': 'int'},
                         'tele_cp_position_successes': {'tim_fields': {'control_panel_position': True}, 'type': 'int'},
                         'climb_all_attempts':         {'tim_fields': {'not': {'climb_time': 0}},       'type': 'int'}}
    expected_output = {'tele_cp_rotation_successes': 2, 'tele_cp_position_successes': 1, 'climb_all_attempts': 2}
...
def test_wonky_schema_for_counts():
    ...
    tims = [{'control_panel_rotation': True,  'control_panel_position': True,  'climb_time': 0},
            {'control_panel_rotation': True,  'control_panel_position': False, 'climb_time': 20},
            {'control_panel_rotation': False, 'control_panel_position': False, 'climb_time': 5}]
    schema_for_counts = {'climb_all_attempts': {'tim_fields': {'climb_success': True}, 'type': 'int'}}
